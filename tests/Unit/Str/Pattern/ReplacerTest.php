...
final class ReplacerTest extends FireHubTestCase {
     ...
     * @return void
     */
    #[TestWith(['the replaced project', 'the firehub project', 'replaced', 'firehub'])]
    #[TestWith(['replaced firehub project', 'the firehub project', 'replaced', 'the'])]
    public function testAll (string $expected, string $string, string $replacement, string $pattern):void {

        self::assertSame(
            $expected,
            new Replacer(new Str($string), $replacement, -1)->all()->custom($pattern)->value()
        );
